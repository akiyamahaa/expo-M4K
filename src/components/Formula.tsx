...
import { EQuizStatus, IQuiz } from '../types/utils'
  ...
  return (
    <HStack space={spacing} alignItems={'center'}>
      ...
      {Object.keys(formula).map((option) => (
        <Box style={{ width: 48 }} alignItems={'center'} key={option}>
          <Text color={status == EQuizStatus.HIDDEN ? '#76c0f5' : '#fff'} fontSize={fontSize}>
            {formula[option]}
          </Text>
        </Box>
      ))}
